...
struct PartitionsData {
		...
		struct cell_t {
			...
			cell_t(const Geometry::Box & boundingBox, uint32_t visibleSet, std::vector<uint32_t> surroundings) :
				bounds(boundingBox), visibleSetId(visibleSet), surroundingIds(surroundings) {
			}
		};
