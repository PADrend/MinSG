...
public:
	...
	struct NodeWrapper {
		...
		NodeWrapper(Util::Reference<GroupNode> _group, const Geometry::Box & _tightBox, const Geometry::Box & _looseBox, uint32_t _depth) :
			group(_group), tightBox(_tightBox), looseBox(_looseBox), depth(_depth) {
		}
	};
